...
namespace Area.Common
{
    ...
    /// ヒトデのリーダー
    /// </summary>
    public class SeastarLeader : MonoBehaviour, IAreaGameManager, ISeastarLeader
    {
        ...
        {
            // ステージIDの取得
            var currentStageDic = AreaGameManager.Instance.SceneOwner.GetSystemCommonCash();
            return currentStageDic[EnumSystemCommonCash.SceneId];
        }

        public void OnStart()
        {
            _quasiAssignForm = new AreaPresenterCommon().LoadSaveDatasCSVAndGetQuasiAssignmentForm();
        }
